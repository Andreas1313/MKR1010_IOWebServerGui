12.12.2025 ad
Arduino MKR WiFi 1010 Pin Control
...
What it does:
It show each Pin A0 - A6, D0 - D12 and RGB LED from MKR1010 at a Webserver- Browser page. (D13 and D14 not, because of Serial Monitor). It is optimized for mobile phone hotspot.
...
- "Digital Output Pullup" and "Digital Output Pullup Negated" should only be used if you know what you are doing.(Open Collector with an internal 20kΩ to 50kΩ pullup resistor). Because of the "floating" pin, it is possible that the Control is different from the Valus. The reason is, that it really reads back the _status of the pin.
...
Not fixed:
- A0 only switch between DAC and Analog Input. If you want to use other, set it at beginning (Example: int _a0Mode = 1, a...)
